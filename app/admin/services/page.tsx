...
import { getAdminServices, deleteService, ServicePage } from '@/lib/api';
...
export default function AdminServiceList() {
    ...
    const loadServices = async () => {
        setLoading(true);
        const data = await getAdminServices({ page, limit: 10, search });
    ...
    const handleDelete = async (id: number) => {
        if (confirm('Are you sure you want to delete this service?')) {
            ...
            if (res.success) {
                ...
                alert(res.message || 'Error deleting service');
            }
        }
    };
            ...
            <div className="space-y-8">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        ...
                        <p className="text-zinc-500 mt-2 font-medium">Manage your service pages and solutions.</p>
                    </div>
                    <Link
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-zinc-100 dark:border-zinc-800">
                                    <th className="px-8 py-6 text-xs font-black text-zinc-400 uppercase tracking-widest">Icon</th>
                                    <th className="px-8 py-6 text-xs font-black text-zinc-400 uppercase tracking-widest">Title</th>
                                    <th className="px-8 py-6 text-xs font-black text-zinc-400 uppercase tracking-widest">Category</th>
                            ...
                            <tbody className="divide-y divide-zinc-50 dark:divide-zinc-800/50">
                                {loading ? (
                                    ...
                                    services.map((service) => (
                                        <tr key={service.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/20 transition-colors group">
                                            <td className="px-8 py-6">
                                                <div className="h-12 w-12 flex items-center justify-center rounded-2xl bg-blue-50 dark:bg-zinc-800 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-zinc-700">
                                                    <i className={`${service.heroIcon || 'fas fa-cog'} text-xl`} />
                                                </div>
                                            </td>
                                            <td className="px-8 py-6">
                                                <div className="flex flex-col gap-1">
                                                    <span className="font-bold text-lg group-hover:text-blue-600 transition-colors">{service.heroTitle}</span>
                                                    <span className="text-zinc-400 text-xs font-medium tracking-tight">/{service.slug}</span>
                                                </div>
                                            </td>
                                            <td className="px-8 py-6">
                                            ...
                                            <td className="px-8 py-6">
                                                <span className={`inline-flex items-center px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider ${service.isPublished
