        ...
        <AdminLayout>
            <div className="max-w-7xl mx-auto pb-20 space-y-10">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                    <div>
                        <h1 className="text-4xl font-black tracking-tight">Product Details</h1>
                        <p className="text-zinc-500 mt-2 font-medium">
                            Manage product demo links and stored credentials. Admin-only vault.
                        </p>
                    </div>
                    <Link
                        href="/admin/product-details/new"
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {[
                        ...
                        <div key={stat.label} className="bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-[28px] p-5 flex items-center gap-4">
                ...
                {/* Empty state */}
                {items.length === 0 ? (
                    <div className="bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-[40px] p-20 text-center">
                        <div className="h-20 w-20 rounded-3xl bg-zinc-50 dark:bg-zinc-800 flex items-center justify-center mx-auto mb-6">
                            <i className="fas fa-cube text-4xl text-zinc-300" />
                        </div>
                        <h3 className="text-2xl font-black mb-3">No product details yet</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        {items.map(item => (
                            <ProductDetailCard
                                ...
                                onEdit={() => router.push(`/admin/product-details/edit/${item.id}`)}
...
function ProductDetailCard({ item, isPasswordRevealed, isDeleting, onShowPassword, onEdit, onDelete }: CardProps) {
    ...
    return (
        <div className={`group bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-[36px] overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 ${!item.isActive ? 'opacity-60' : ''}`}>
            {/* Thumbnail / Header */}
            ...
            <div className="p-6 space-y-4">
                {/* Title */}
                <h3 className="text-lg font-black leading-tight truncate" title={item.title}>{item.title}</h3>
                ...
                <div className="flex items-center gap-2 pt-1">
                    <button
                        onClick={onEdit}
                        className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl font-black text-xs uppercase tracking-wider bg-zinc-50 dark:bg-zinc-800 hover:bg-blue-600 hover:text-white border border-zinc-100 dark:border-zinc-700 hover:border-blue-600 transition-all"
                    >
